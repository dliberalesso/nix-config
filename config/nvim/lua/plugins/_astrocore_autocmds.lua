    ...
    commands = {
      AstroVersion = {
        function()
          ...
          if version then
            require("astrocore").notify(("Version: *%s*"):format(version))
          end
        end,
      ...
      create_dir = {
        {
          ...
          callback = function(args)
            if not require("astrocore.buffer").is_valid(args.buf) then
              return
            end
      ...
      file_user_events = {
        {
          ...
          callback = function(args)
            if vim.b[args.buf].astrofile_checked then
              return
            end
          ...
          callback = function(args)
            ...
            if vim.tbl_get(astrocore.config, "features", "highlighturl") then
              ...
              vim.tbl_map(function(win)
                if vim.w[win].highlighturl_enabled then
                  astrocore.delete_url_match(win)
                end
            ...
            if illuminate_avail then
              illuminate.stop_buf(args.buf)
            end
            local rainbow_avail, rainbow = pcall(require, "rainbow-delimiters") -- disable rainbow-delimiters
            if rainbow_avail then
              rainbow.disable(args.buf)
            end
          end,
        },
      },
